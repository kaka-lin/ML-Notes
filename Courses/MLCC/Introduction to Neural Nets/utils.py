...
def train_nn_regression_model(
    ...
    @returns:               A `DNNRegressor` object trained on the training data.
    ...
    for period in range(0, periods):
        ...
        training_predictions = dnn_regressor.predict(predict_training_input_fn)
        training_predictions = np.array([item['predictions'][0] for item in training_predictions])
        
        validation_predictions = dnn_regressor.predict(predict_validation_input_fn)
    ...
    plt.tight_layout()
    plt.plot(training_root_mean_squared_error, label="training")
    plt.plot(validation_root_mean_squared_error, label="validation")
    plt.legend()
