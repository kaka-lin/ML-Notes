...
from keras.layers import Conv2D, MaxPool2D, Flatten, ZeroPadding2D, AveragePooling2D
...
if __name__ == '__main__':
    ...
    model = Sequential()
    model.add(Conv2D(64, (5, 5), input_shape=(48, 48, 1), activation='relu', padding='valid'))
    ...
    opt = Adadelta(lr=0.1, rho=0.95, epsilon=1e-8)
    model.compile(loss='categorical_crossentropy', optimizer=opt,
                  metrics=['accuracy'])
    
    model.summary()

    save_best = callbacks.ModelCheckpoint('hw3_best_model_2.h5', monitor='val_acc', 
